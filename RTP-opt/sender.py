    ...
    def _send_start_packet(self):
        ...
        while True:
            ...
            if len(self.shared_dict) > 0:
                self.base_seq_num = self.shared_dict["newest_ack"].pop(0)
                if self.base_seq_num == 0:
        ...
        while True:
            ...
            if len(self.shared_dict) > 0:
                for seq_num in self.buffer:
                    if seq_num in self.shared_dict["newest_ack"]:
                        print("ACK received for packet:", seq_num)
                        self.received_ack.add(seq_num)
                    else:
                        print("ACK not received for packet:", seq_num)
                else:
                    # Remove the acknowledged packets from the buffer
                    for seq_num in self.received_ack:
                        if seq_num in self.buffer:
                            del self.buffer[seq_num]

                    self.shared_dict["newest_ack"] = []
                    if self.base_seq_num in self.received_ack:
                        self.received_ack.remove(self.base_seq_num)
                        self.base_seq_num = min(self.received_ack)
                        self.received_ack = {ack for ack in self.received_ack if
                                             ack > self.base_seq_num}
            else:
    ...
    def _wait_for_ack(self):
        ...
        while True:
            ...
            if pkt_header.type == PACKET_TYPE.ACK and self._compare_checksum(msg, pkt_header) \
                    and pkt_header.seq_num >= self.base_seq_num:
                temp = self.shared_dict.get("newest_ack", [])
                temp.append(pkt_header.seq_num)
                self.shared_dict["newest_ack"] = temp
...
def main():
    ...
    sender = RTPSenderOptimized(args.receiver_ip, args.receiver_port, args.window_size)
    sender.send(message)
